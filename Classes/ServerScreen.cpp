...
bool ServerScreen::init(){
    ...
    state = Label::createWithTTF("Creating Server...", "fonts/Marker Felt.ttf", 25);
    ...
    ipLabel = Label::createWithTTF("IP", "fonts/Marker Felt.ttf", 16);
    ...
    portLabel = Label::createWithTTF("PORT", "fonts/Marker Felt.ttf", 16);
    ...
    playerLabel = Label::createWithTTF("", "fonts/Marker Felt.ttf", 20);
        ...
        if ((fd=socket(AF_INET, SOCK_STREAM, 0)) > 0){
            ...
            if(bind(fd,(struct sockaddr*)&server, sizeof(struct sockaddr))<=0) {
                if(listen(fd,1) <=0) {
                    ...
                    }else if(!IpScreen.compare("0.0.0.0")){
                        screen->connectionFailed();
                    }else{
